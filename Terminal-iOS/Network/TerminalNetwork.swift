...
class TerminalNetwork {
    ...
    static func getNewStudyListForKey(_ keyValue: String, completionHandler: @escaping ([Study]) -> ()) {
        let query = "3.35.154.27:3000/v1/study/paging/list?values=\(keyValue)"
        var studyArr: [Study] = []
...
//                    do {
                        for index in data {
//                            if index["title"].string != nil {
                                let temp = Study(title: index["title"].string!, subTitle: index["introduce"].string!, location: "ㄹㅇㄹㅇ", date: index["created_at"].string!, managerImage: UIImage(named: "leehi")!, mainImage: UIImage(named: "mainImage")!)
                                studyArr.append(temp)
                                completionHandler(studyArr)
//                            }
                        }
